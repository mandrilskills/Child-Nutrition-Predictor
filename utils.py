...
def _comparative_table(data: list, styles) -> Table:
    """Renders the WHO vs Actual metrics table."""
...
def generate_pdf_report(
    ...
    # Filter out complex agentic data to just show clean KV rows
    ...
    # SECTION 4 — AI Clinical Analysis
    story.append(_section_banner("4.  AI Clinical Analysis  —  Explainer Agent", styles))
    story.append(Spacer(1, 3 * mm))
    story.append(KeepTogether(_ai_text_block(explainer_text, styles) or [Paragraph("No output generated.", styles["body"])]))
    story.append(Spacer(1, 5 * mm))

    # SECTION 5 — Socio-Economic Intervention
    story.append(_section_banner("5.  Socio-Economically Constrained Intervention  —  Policy Agent", styles))
    story.append(Spacer(1, 3 * mm))
    story.append(KeepTogether(_ai_text_block(unicef_text, styles) or [Paragraph("No output generated.", styles["body"])]))
    story.append(Spacer(1, 5 * mm))

    # SECTION 6 — CMO Safety Audit
